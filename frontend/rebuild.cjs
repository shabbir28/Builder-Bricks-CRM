...
const Properties = () => {
  ...
  const fetchProperties = async () => {
    try {
      ...
      if (r.data.success) {
        // Ensure accurate sorting by original import logic if possible or rely on natural insertion order
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <F label="Listed By *">
            <select className={inp} value={form.listedBy} onChange={e => setForm(p => ({ ...p, listedBy: e.target.value }))} required>
              ...
              {agents.map(a => <option key={a.id} value={a.id} style={{ background: '#0e1015' }}>{a.name}</option>)}
            </select>
          </F>
        </div>
